...
local use_canvas_viewport = true
...
function get_mouse_pos()
    ...
    elseif ffi.os == "Linux" then
        if x11_lib ~= nil and x11_display ~= nil and x11_root ~= nil and x11_mouse ~= nil then
            if x11_lib.XQueryPointer(x11_display, x11_root, x11_mouse.root_win, x11_mouse.child_win, x11_mouse.root_x,
                x11_mouse.root_y, x11_mouse.win_x, x11_mouse.win_y, x11_mouse.mask) ~= 0 then
...
function refresh_sceneitem(find_newest)
    ...
    if sceneitem ~= nil then
                    ...
                    if name ~= CROP_FILTER_NAME and name ~= "temp_" .. CROP_FILTER_NAME then
                        ...
                        if settings ~= nil then
                            if not obs.obs_data_get_bool(settings, "relative") then
                                ...
                                log("Found existing relative crop/pad filter (" .. name .. "). Applying settings " ..
                                        format_table(zoom_info.source_crop_filter))
                            else
        ...
        if not found_crop_filter and
            (sceneitem_crop_orig.left ~= 0 or sceneitem_crop_orig.top ~= 0 or sceneitem_crop_orig.right ~= 0 or
                sceneitem_crop_orig.bottom ~= 0) then
...
function get_target_position(zoom)
    ...
    local viewport_width = zoom.source_size.width
    local viewport_height = zoom.source_size.height

    local canvas = get_canvas_size()

    local canvas_width = canvas.width
    local canvas_height = canvas.height

    -- MODE VERTICAL CANVAS
    if use_canvas_viewport then

        local source_aspect = zoom.source_size.width / zoom.source_size.height
        local canvas_aspect = canvas_width / canvas_height

        -- hitung area viewport portrait di dalam source landscape
        if source_aspect > canvas_aspect then

            viewport_height = zoom.source_size.height
            viewport_width = viewport_height * canvas_aspect

        else

            viewport_width = zoom.source_size.width
            viewport_height = viewport_width / canvas_aspect

        end
    end

    local new_size = {
        width = viewport_width / zoom.zoom_to,
        height = viewport_height / zoom.zoom_to
    }
    ...
    if pressed then
        ...
        if zoom_state == ZoomState.ZoomedIn or zoom_state == ZoomState.None then
            if zoom_state == ZoomState.ZoomedIn then
                ...
                locked_last_pos = nil
                zoom_target = {
                    crop = crop_filter_info_orig,
                    c = sceneitem_crop_orig
                }
        ...
        if zoom_state == ZoomState.ZoomingOut or zoom_state == ZoomState.ZoomingIn then
            ...
            if is_following_mouse then
                ...
                if not use_follow_outside_bounds then
                    if zoom_target.raw_center.x < zoom_target.crop.x or zoom_target.raw_center.x > zoom_target.crop.x +
                        zoom_target.crop.w or zoom_target.raw_center.y < zoom_target.crop.y or zoom_target.raw_center.y >
                        zoom_target.crop.y + zoom_target.crop.h then
                ...
                if not skip_frame then
                    ...
                    if locked_center == nil and
                        (zoom_target.crop.x ~= crop_filter_info.x or zoom_target.crop.y ~= crop_filter_info.y) then
        ...
        if zoom_time >= 1 then
            ...
            elseif zoom_state == ZoomState.ZoomingIn then
                ...
                if is_following_mouse and follow_border < 50 then
                    zoom_target = get_target_position(zoom_info)
                    locked_center = {
                        x = zoom_target.clamped_center.x,
                        y = zoom_target.clamped_center.y
                    }
                    log("Cursor stopped. Tracking locked to " .. locked_center.x .. ", " .. locked_center.y)
                end
            end
...
function script_properties()
    ...
    local refresh_sources = obs.obs_properties_add_button(props, "refresh", "Refresh zoom sources", function()
        populate_zoom_sources(sources_list)
        monitor_info = get_monitor_info(source)
        return true
    end)
    ...
    local safezone_sense = obs.obs_properties_add_int_slider(props, "follow_safezone_sensitivity", "Lock Sensitivity",
        1, 20, 1)
...
function script_update(settings)
    ...
    if source_name ~= old_source_name or use_monitor_override ~= old_override or monitor_override_x ~= old_x or
        monitor_override_y ~= old_y or monitor_override_w ~= old_w or monitor_override_h ~= old_h or monitor_override_sx ~=
        old_sx or monitor_override_sy ~= old_sy or monitor_override_w ~= old_dw or monitor_override_h ~= old_dh then
        monitor_info = get_monitor_info(source)
    end
end
